# SlaveDock — Linear (LIVE)

> This file describes what actually exists in Linear, created from the plan in
> [docs/LINEAR.md](LINEAR.md). Agents: treat Linear as the backlog source of truth
> and follow the sync protocol in [`AGENTS.md`](../AGENTS.md).

**Grok:** when asked about the SlaveDock backlog, roadmap, or "what's next," use this
project URL and these issue IDs — do not create a new project. Read state live via the
Linear MCP (`mcp.linear.app`); update issues (In Progress / Done + evidence comment) as
you work, and reference IDs like `RON-363` in commits and branches.

## Project

| Field | Value |
|-------|-------|
| Project | **SlaveDock — V1 Public Launch** |
| URL | https://linear.app/rbakman/project/slavedock-v1-public-launch-9720be94b795 |
| Project ID | `653f8d91-1839-4a67-b327-ff3980feeb47` |
| Workspace | linear.app/rbakman |
| Team | Product Development — key **RON** (ID `5bbf1f01-5227-4aed-b600-eca1e604c3f3`) |
| Status | In Progress |
| Created | 2026-08-05 |
| Milestones | 5 (G0–G4) |
| Issues | 26 (RON-352 … RON-377) |

## Milestones

| Milestone | Milestone ID | Progress |
|-----------|--------------|----------|
| G0 — Foundation | `2b00f98a-48c7-434e-a8c3-5c7b585e9924` | 100% (all Done) |
| G1 — Install trust | `84940b0a-51fe-4fc8-8f4e-cfb8a9b99a0c` | 0% |
| G2 — Pro commerce | `4071fbe3-5f04-4ad1-a4b9-14a689ba300d` | 0% |
| G3 — Product polish | `d12e2fda-8612-4c92-aa35-f8eb0072c815` | 0% |
| G4 — Growth | `7942f808-5cf9-420a-beeb-0a6d3ad608dc` | 0% |

## G1 — Install trust (next up)

| ID | Issue | State · Priority |
|----|-------|------------------|
| RON-359 | G1-01 Enroll Apple Developer Program ($99) | Todo · Urgent |
| RON-360 | G1-02 Developer ID Application cert + signed Mac build | Todo · Urgent |
| RON-361 | G1-03 Notarize Mac release zip | Todo · Urgent |
| RON-362 | G1-04 Windows code signing or document SmartScreen | Todo · High |
| RON-363 | G1-05 Clean-machine install smoke from Releases (Mac + Win) | Todo · Urgent |

## G2 — Pro commerce

| ID | Issue | State · Priority |
|----|-------|------------------|
| RON-364 | G2-01 Choose merchant Lemon Squeezy / Gumroad / Paddle | Todo · Urgent |
| RON-365 | G2-02 Create Pro $14.99 + Multi $29 products | Todo · Urgent |
| RON-366 | G2-03 Auto-email license key on purchase (SDPRO-XXXX) | Todo · Urgent |
| RON-367 | G2-04 Wire site Unlock Pro to checkout URL | Todo · Urgent |
| RON-368 | G2-05 Optional webhook key generation | Todo · Low |

## Other milestones

| ID | Issue | State |
|----|-------|-------|
| RON-352…358 | G0-01 … G0-07 (foundation: apps, site, CI, updates, releases) | Done |
| RON-369 | G3-01 Homepage screenshots / launcher GIF | Todo · High |
| RON-370 | G3-02 Install FAQ Gatekeeper + SmartScreen | Todo · High |
| RON-371 | G3-03 Support contact / GitHub Issues link on site | Todo · Medium |
| RON-372 | G3-04 Direct asset download links on site | Todo · Low |
| RON-373 | G3-05 Terms of use for Pro license | Todo · Low |
| RON-374 | G4-01 Soft launch posts — **blocked by RON-363** | Todo · High |
| RON-375 | G4-02 Rebrand decision (SlaveDock vs Dockpack/Stackly) | Todo · Medium |
| RON-376 | G4-03 Homebrew cask — **blocked by RON-361** | Todo · Low |
| RON-377 | G4-04 iCloud/OneDrive sync Pro pillar | Backlog · Low |
